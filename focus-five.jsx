  ...
  return (
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={styles.selfHostBox}>
          <p style={{ margin: "0 0 4px", fontWeight: 600, fontSize: 13 }}>Self-host reminder cron</p>
          <p style={{ margin: 0, fontSize: 12.5, opacity: 0.8 }}>
            Set up a cron job on your server that hits your Focus Five endpoint
            and sends a notification via ntfy with your most neglected task.
          </p>
        </div>
...
// Check-in modal
function CheckInModal({ show, tasks, onClose, onReorder, onRemove }) {
...
export default function FocusFive() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("focus-five-data");
        if (res && res.value) {
          const data = JSON.parse(res.value);
          setTasks(data.tasks || []);
          setNtfyConfig(data.ntfyConfig || ntfyConfig);
        }
      } catch (e) {
  ...
  // Save to storage
  const persist = useCallback(async (newTasks, newNtfy) => {
    try {
      await window.storage.set("focus-five-data", JSON.stringify({
        tasks: newTasks,
        ntfyConfig: newNtfy || ntfyConfig,
      }));
    } catch (e) {
  ...
  const swapTask = (removeId) => {
    const filtered = tasks.filter((t) => t.id !== removeId);
    const task = {
      id: generateId(),
      title: newTitle.trim(),
      progress: 0,
  ...
  const saveNtfy = (config) => {
    setNtfyConfig(config);
    persist(tasks, config);
  };
      ...
      {tasks.length > 0 && (
        <div style={styles.footer}>
          ...
          <div style={styles.footerDesc}>
            Add this to your server's crontab to get daily nudges about neglected tasks.
            Pair with <a href="https://ntfy.sh" target="_blank" rel="noopener" style={{ color: "#5a8a6a" }}>ntfy</a> for
            phone push notifications.
          </div>
          <code style={styles.footerCode}>
            {`# Crontab: 0 ${ntfyConfig.reminderHour || 9} * * * /path/to/focus-reminder.sh\ncurl -d "Time to check your Focus Five!" \\
  ${ntfyConfig.server || "https://ntfy.sh"}/${ntfyConfig.topic || "your-topic"}`}
          </code>
        </div>
      )}
